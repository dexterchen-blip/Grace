...
exec env -i \
  ...
  AIAGENT_EXCHANGE_DAYTIME="${AIAGENT_EXCHANGE_DAYTIME:-}" \
  HF_HOME="$HF_HOME" \
  HF_HUB_OFFLINE=1 \
  GRACE_EWC="${GRACE_EWC:-1}" \
  GRACE_EWC_LAMBDA="${GRACE_EWC_LAMBDA:-1e-3}" \
  GRACE_EWC_MODE="${GRACE_EWC_MODE:-B}" \
  GRACE_SLEEP_DELTA="${GRACE_SLEEP_DELTA:-0.03}" \
  GRACE_NO_DIALOGUE="${GRACE_NO_DIALOGUE:-0}" \
  GRACE_SYNTHETIC="${GRACE_SYNTHETIC:-0}" \
  GRACE_CURIOSITY="${GRACE_CURIOSITY:-0}" \
  LANG="en_US.UTF-8" LC_ALL="en_US.UTF-8" TZ="${AIAGENT_TZ:-Asia/Shanghai}" \
  "$@"

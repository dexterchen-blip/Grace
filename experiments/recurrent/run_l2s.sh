...
restore_8100() { [ "$RESTORE" -eq 1 ] && { launchctl bootstrap gui/501 "$PLIST" 2>/dev/null; sleep 12; curl -s --max-time 5 http://127.0.0.1:8100/v1/models >/dev/null && echo "[L2S] 8100 已恢复 ✓"; }; }
trap restore_8100 EXIT
stop_8100
HF_HUB_OFFLINE=1 "$PY" "$HERE/../lora/eval_l2splice.py"
exit $?

...
def add_event(event: str, ts: float = None, person: str = "", entity: str = "",
              ...
              confidence: str = "high", evidence: str = "", event_id: str = "") -> int:
    """摄入事件 → 自传体叙事矩阵节点（2026-08-29 幻觉抑制：必须带证据与置信度）。

    confidence: high=直接来自 L0 真实记录 / medium=加工层产物 / low=模型生成(仅叙事,禁作事实)。
    ★2026-09-05 O3 修复(9/5 实码: L3 侧压根没有缝合键——图谱侧 ev-d{day}-{i} 单方面存在):
      新增 event_id 参数(与 mood_graph 同键), 表缺列自动迁移 → L3↔图谱 event 团簇缝合打通。
    """
    con = _conn(db)
    # ★O3: event_id 列自动迁移(旧库无此列)
    _cols = [r[1] for r in con.execute("PRAGMA table_info(autobiography)").fetchall()]
    if "event_id" not in _cols:
        con.execute("ALTER TABLE autobiography ADD COLUMN event_id TEXT DEFAULT ''")
    cur = con.execute(
        "INSERT INTO autobiography(ts,event,person,entity,emotion,relation,self_eval,evidence,confidence,event_id) "
        "VALUES(?,?,?,?,?,?,?,?,?,?)",
        (ts or time.time(), event[:160], person, entity, emotion, relation, self_eval,
         evidence[:200], confidence, event_id))
    con.commit()
    con.close()
    return cur.lastrowid


def query_event_id(event_id: str, db: str = None) -> list[dict]:
    """★O3 配套: 按 event_id 查 L3 节点(与 mood_graph.query_event 对偶, 团簇缝合的 L3 侧读路径)。"""
    con = _conn(db)
    rows = con.execute(
        "SELECT ts,event,person,entity,emotion,relation,self_eval,confidence FROM autobiography "
        "WHERE event_id=? ORDER BY ts", (event_id,)).fetchall()
    con.close()
    return [{"ts": r[0], "event": r[1], "person": r[2], "entity": r[3],
             "emotion": r[4], "relation": r[5], "self_eval": r[6], "confidence": r[7]} for r in rows]

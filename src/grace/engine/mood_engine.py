...
def derive(events: list[dict], db: str = None, ts: float | None = None) -> dict:
    ...
    #   intensity = prev×0.7 + 0.15 + (delta_mapped-0.5)×1.0（clamp 0-1）
    #   ★2026-09-03 X轮校准(std 0.0136<0.05 恒平失真)：镜像增益 0.6→1.0（原作 0.9-1.0 取 1.0，放大主人情绪镜像）
    #   中性稳态=0.5；代表性好日(Δm≈0.9) +0.40/日、坏日 -0.40/日；极端日跳变上限≈0.5 由 clamp 收
    intensity = round(max(0.0, min(1.0,
                     base * config.MOOD["decay"] + 0.15 + (delta_mapped - 0.5) * 1.0)), 3)

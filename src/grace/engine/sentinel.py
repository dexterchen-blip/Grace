...
def judge_with_5b(hits: list[dict]) -> list[dict]:
    ...
    try:
        ...
        # ★2026-09-07 修: 原按 repo id 加载报 "Cannot find an appropriate cached snapshot"
        #   (HF 缓存只有 refs 空壳)——本地完整权重在 models/sentinel-3b(8/29 下载, 2.0G)。
        #   直接加载本地路径; AIAGENT_SENTINEL_MODEL 可覆盖。
        _m = _os.environ.get("AIAGENT_SENTINEL_MODEL",
                             _os.path.join(SENTINEL_DIR, "..", "..", "models", "sentinel-3b"))
        model, tok = load(_m)

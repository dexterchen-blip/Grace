...
def build_messages(internal: dict) -> list[dict]:
    ...
    ★2026-09-04 铁律(脑科学 Levelt 回查 + 9/2 定稿): 本模块输入 = **preverbal message 状态**
      (概念化层产出, 非语言形式), 绝不接收内心独白原文(think/cog)——内语是元认知/ToM 发生地,
      应保持自由只观测; Broca 重新编码而非倒出思考(本文件 docstring 原文)。think 文本不进表达。"""
    parts = []
...
def monitor(msg: str) -> str:
    ...
    if any(k in msg for k in ("姐姐大人", "拉姆", "昴君", "巴鲁斯")):
        return ""
    # ★2026-09-04 审计修复(dialogue 验证实锤): 小说/舞台旁白体漏网——
    #   「(听到'主人'…手微微一顿,肩膀轻轻颤动,眼神中闪过一丝狂喜)」——第三人称动作+心理描写
    #   是对外说话不该有的形态(她当面说话不念旁白)。特征: 括号内心理/动作旁白、表情神态描写。
    if _re.search(r"[（(][^）)]{0,44}(听到|微微|颤动|眼神|神情|肩膀|攥紧|瞳孔|呼吸|心跳|沉默|闪过)", msg):
        return ""
    if _re.search(r"(眼神|神情|语气|声音|嘴角|瞳孔)[^，。！？]{0,10}(闪过一丝|带着|藏着|透出|微不可察|强压|压住|恢复平静)", msg):
        return ""
    return msg[:120]
